...
public class TieJian_62 {
	...
	public TieJian_62() {
		...
		meta.setDisplayName(ChatColor.RESET + ""+ChatColor.BLUE+"海盗之剑");
//		meta.addEnchant(Enchantment.DURABILITY, 1, false);
		List<String> lore = new ArrayList<>();
		lore.add(ChatColor.RESET + " ");
		lore.add(ChatColor.RESET + "稀有度：" + ChatColor.BLUE + "强化");
		lore.add(ChatColor.RESET + "所需等级："+ChatColor.RED+"55");
		level = 55;
		lore.add(ChatColor.RESET + "绑定：自由交易");
		lore.add(ChatColor.RESET+ "最大耐久：400");
		lore.add(ChatColor.RESET+ "基础伤害：7");
		lore.add(ChatColor.RESET+ "基础攻速：0.625");
		meta.setLore(lore);
		meta.addAttributeModifier(Attribute.GENERIC_ATTACK_SPEED, new AttributeModifier(new UUID(1, 1), "1", 0.35,
				AttributeModifier.Operation.ADD_SCALAR, EquipmentSlot.HAND));
		meta.addAttributeModifier(Attribute.GENERIC_ATTACK_DAMAGE, new AttributeModifier(new UUID(1, 2), "2", 3.5,
				AttributeModifier.Operation.ADD_NUMBER, EquipmentSlot.HAND));
		if (meta instanceof Damageable) {
			((Damageable) meta).setDamage(-149);
		}
		...
		recipe2.shape(5, TieJian_52.itemStack);
